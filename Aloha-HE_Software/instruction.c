...
uint64_t getFFTTransformationInstructionWord(uint8_t isDIF, uint8_t current_n)
{
	assert(isDIF < (1<<1));
	return (1ull<<42) | ((uint64_t)current_n << 40) | (1<<4) | (isDIF<<3) | OPC_TRANSFORMATION;
}
...
uint64_t getNTTTransformationInstructionWord(uint8_t isDIF, uint8_t log_q, uint8_t modulus_rom_index, uint32_t qm, uint8_t current_n)
{
	assert(isDIF < (1<<1));
	assert(log_q < (1<<4));
	assert(modulus_rom_index < 18);
	assert(qm < (1<<17));
	qm = (-qm) & ((1<<17) - 1);
	return (1ull<<42) | ((uint64_t)current_n << 40) | ((modulus_rom_index&0x10)<<26) | (qm<<13) | ((modulus_rom_index&0xf)<<9) | (log_q<<5) | (isDIF<<3) | OPC_TRANSFORMATION;
}
...
// @param qm: The 17-bit value of the modulus q = 2^(log_q[i]+46) - (qm << 24) + 1
uint64_t getRNSInstructionWord(uint16_t log_scale, uint8_t log_q, uint8_t modulus_rom_index, uint32_t qm, uint8_t current_n)
{
	assert(scale < (1<<12));
	assert(log_q < (1<<4));
	...
	qm = (-qm) & ((1<<17) - 1);
	return (1ull<<42) | ((uint64_t)current_n << 40) | (scale_high<<33) | (scale_low<<30) | (qm<<13) | (modulus_rom_index<<9) | (log_q<<5) | (scale_mid<<3) | OPC_RNS;
}
...
//				 A value of 0 corresponds to 46-bit modulus, 1 -> 47-bit, ..., 8 -> 54-bit modulus
// @param qm: The 17-bit value of the modulus q = 2^(log_q[i]+46) - (qm << 24) + 1
uint64_t getI2FInstructionWord(int16_t log_scale, uint8_t log_q, uint32_t qm, uint8_t current_n)
{
	...
	qm = (-qm) & ((1<<17) - 1);
	return (1ull<<42) | ((uint64_t)current_n << 40) | (scale_high<<33) | (scale_low<<30) | (qm<<13) | (log_q<<5) | (scale_mid<<3) | OPC_I2F;
}
...
//				 A value of 0 corresponds to 46-bit modulus, 1 -> 47-bit, ..., 8 -> 54-bit modulus
// @param qm: The 17-bit value of the modulus q = 2^(log_q[i]+46) - (qm << 24) + 1
uint64_t getPWMInstructionWord(uint8_t log_q, uint32_t qm, uint8_t current_n)
{
	assert(log_q < (1<<4));
	assert(qm < (1<<17));
	qm = (-qm) & ((1<<17) - 1);
	return (1ull<<42) | ((uint64_t)current_n << 40) | (qm<<13) | (log_q<<5) | OPC_PWM;
}

// Returns an instruction word to perform Projection
uint64_t getProjectInstructionWord(uint8_t current_n)
{
	return (1ull<<42) | ((uint64_t)current_n << 40) | OPC_PROJECT;
}
